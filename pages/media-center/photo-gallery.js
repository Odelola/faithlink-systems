...
const [index, setIndex] = useState("");
    const galleryData = [
        {
            category: "",
            imgUrl: "/pages/photo-gallery/IMG-20240425-WA0041.jpg",
            title: "First Site",
        },
        {
            category: "",
            imgUrl: "/pages/photo-gallery/IMG-20240425-WA0048.jpg",
            title: "Second Site",
        },
        {
            category: "",
            imgUrl: "/pages/photo-gallery/IMG-20240425-WA0053.jpg",
            title: "Third Site",
        },
        {
            category: "",
            imgUrl: "/pages/photo-gallery/IMG-20240425-WA0054.jpg",
            title: "Fourth Site",
        },
        {
            category: "",
            imgUrl: "/pages/photo-gallery/IMG-20240425-WA0055.jpg",
            title: "Fifth Site",
        },
    ]
    ...
    const PhotoGalleryCard = ({ galleryDetail }) => {
        return (
            galleryDetail.map((item, index) => {
                return (
                    <div className="flex flex-col border-[#ccdbe7] border-b border-r [&:nth-child(5n)]:border-r-0 p-7  " data-filter={item.category} key={index}>
                        <div className="flex justify-center items-center min-h-32  overflow-hidden mb-4 relative group">
                            <a href="javascript:;" data-fancybox="gallery" data-src={item.imgUrl}>
                                <img src={item.imgUrl} alt={item.title} loading="lazy" className="group-hover:scale-110" />
                            </a>
                        </div>
                        <h3 className="text-textGray">{item.title.toUpperCase()}</h3>
                        <div className="logos__action">
                            <a href={item.imgUrl} download="" className="flex justify-center items-center w-28 h-8 border border-secondary text-secondary fill-secondary hover:bg-secondary hover:text-white gap-x-4 hover:fill-white">
