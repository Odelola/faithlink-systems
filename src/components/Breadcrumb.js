...
function Breadcrumb({ leadingRoutes = ["homepage"] }) {
    ...
    function GeneratePath({ leadingRoutes }) {
        return (
            leadingRoutes.map((item, index) => {
                return (
                    <li itemProp="itemListElement" itemScope itemType="http://schema.org/ListItem" key={item}>
                        <Link itemType="http://schema.org/Thing" itemProp="item" href={(index == 1) ? "#" : getRoute[item] } className=''>
                            <span itemProp="name" className='text-white leading-3'>{item.toUpperCase()}</span>
                        </Link>
                        <meta itemProp="position" content="1" />
                        <svg xmlns="http://www.w3.org/2000/svg" width="4" height="7" viewBox="0 0 4 7" fill="none" className='inline-flex mx-[10px] max-xs:hidden'>
    ...
    return (
        <ol className="absolute flex items-center bg-secondary h-[50px] pl-10 pr-[15px] max-md:px-2 md:min-w-[600px] bottom-[-25px] m-0 max-md:w-full max-md:left-0 max-xs:gap-x-1" itemScope itemType="http://schema.org/BreadcrumbList">
            ...
            <li itemProp="itemListElement" itemScope itemType="http://schema.org/ListItem">
                <span itemProp="name" className="text-primary text-[10px]">{nameOfRoute.includes("-") ? nameOfRoute.split("-").join(" ") : nameOfRoute}</span>
