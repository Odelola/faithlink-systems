...
function Navbar() {
  ...
  function MenuLinks({ linkName, subLinks }) {
    ...
    return (
      <li className="relative group px-4 py-4 max-md:px-0">
        {/* <div> */}

        <Link href={"#"} className="font-terminamedium font-medium w-full max-md:flex max-md:items-center max-md:justify-between max-md:text-white">
          {linkName}
  ...
  function SubMenu({ subLinks, className = "" }) {
    return (
      <ul className={"absolute left-0 z-50 invisible opacity-0 group-hover:visible group-hover:opacity-100 min-w-[300px] h-0 group-hover:h-[580px] max-md:hidden" + className}>
        {subLinks?.map((item, index) => (
          <li key={item} className={`opacity-0 invisible translate-x-[-8px] group-hover:visible group-hover:opacity-100 group-hover:delay-${75 * index}`} style={{ transition: "all .3s ease-in-out" }}>
  ...
  function InnerSubMenu({ subLinks = ["power sector", "engineering services"] }) {
    return (
      ...
      <header className={`fixed w-full z-[1000] bg-white transition-transform h-20 flex items-center text-textGray`}>
        ...
        <div className="container flex justify-between items-center">
          ...
          <div className="flex items-center gap-x-4">
            ...
            <nav className="max-md:w-full max-md:h-screen max-md:fixed max-md:left-0 max-md:top-0 max-md:overflow-y-scroll max-md:hidden" aria-label="Main">
              <ul className="flex items-center max-md:flex-col max-md:w-full max-md:h-full max-md:py-[10%] max-md:justify-between max-md:items-start">
                ...
                <MenuLinks linkName="ACTIVITY FIELDS" subLinks={["power sector", "infra development", "engineering services", "renewable energy", "fibre optics"]} />
                <MenuLinks linkName="SUSTAINABILITY" subLinks={["safe workplace", "quality management system"]} />
            ...
            <nav className="pt-4">
              ...
              <ul className={`mx-7 mt-16`}>
                ...
                <li className='border-b border-[rgba(225,225,225,.5)]'>
                  ...
                  {mobileToggleId == 1 && <MobileSubMenu subLinks={["power sector", "infra development", "engineering services", "pass / eaas", "fibre optics"]} mobileToggle={mobileToggle} setToggle={setToggle} setMobileToggle={setMobileToggle} />}
                </li>
                <li className='border-b border-[rgba(225,225,225,.5)]'>
                  <Link href="/" onClick={() => {setMobileToggle(prev => !prev); setMobileToggleId(prev => prev = 1);}} className='flex justify-between items-center text-white py-5 text-[16px]'>
                    SUSTAINABILITY
                    <svg width="6" height="11" viewBox="0 0 6 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      {mobileToggleId == 2 && <MobileSubMenu subLinks={["safe workplace", "quality management system"]} mobileToggle={mobileToggle} setToggle={setToggle} setMobileToggle={setMobileToggle} />}
                </li>
                <li className='border-b border-[rgba(225,225,225,.5)]'>
