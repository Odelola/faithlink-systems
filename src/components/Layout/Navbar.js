...
function MobileSubMenu({ subLinks, mobileToggle, setMobileToggle, setToggle }) {
  return (
    <ul className={`mx-7 mt-0 absolute top-0 w-full h-full bg-primary ${mobileToggle ? "left-0" : "left-[200%]"}`}>
      <li onClick={() => { setMobileToggle(prev => prev = false) }} className={`flex items-center gap-x-1 border-b py-5 border-[rgba(225,225,225,.5)]`}>
