...
function PageSubheader({ imgUrl, alt, pageName, pageText = "", leadingRoutes, greenOverlay = false }) {
  const greenOverlayStyles = "after:content-[''] after:bg-primary after:opacity-70 after:absolute after:left-0 after:top-0 after:w-full after:h-full"
  return (
    <article className='relative h-[400px] xs:max-md:h-[200px] md:max-lg:h-[300px] w-full '>
      {/* <div className="bg-primary opacity-50 absolute left-0 top-0 w-full h-full z-50">

</div> */}
      <div className="container h-full xs:max-md:p-0">
        <picture className={`flex h-full relative ${greenOverlay && greenOverlayStyles}`}>
          {/* after:bg-primary after:opacity-50 after:absolute after:left-0 after:top-0 after:w-full after:h-full after:z-50 */}
